...
namespace Sprint03
{
    public class DodongoSM : IStateMachine
    {
        private Game1 Game;
        ...
        public DodongoSM(Monster DoDongo, Game1 game)
        {
            Game = game;
            Self = DoDongo;
        }
        ...
        public void AttackState()
        {
            if (Math.Abs(Self.Position.X - Game.Link.Position.X) < Math.Abs(Self.Position.Y - Game.Link.Position.Y))
            {
                if (Game.Link.Position.Y < Self.Position.Y)
                {
                    ...
                    Self.Sprite.ChangeSpriteAnimation("DoDongoUp");
                    direction = "Up";
            ...
            else
            {
                if (Game.Link.Position.X < Self.Position.X)
                {
                    ...
                    Self.Sprite.ChangeSpriteAnimation("DoDongoLeft");
                    direction = "Left";
                ...
                else if (Game.Link.Position.X > Self.Position.X)
                {
                    ...
                    Path.Y = Self.Position.Y;
                    Self.Sprite.ChangeSpriteAnimation("DoDongoRight");
                    direction = "Right";
        ...
        public void DamagedState()
        {
            ...
            if (Timer == 1)
            {
                Self.Sprite.ChangeSpriteAnimation("DoDongo" + direction + "Damaged");
            ...
            {
                Timer = 0;
                Self.Sprite.ChangeSpriteAnimation("DoDongo" + direction);
                Reset();
                Self.State = States.MonsterState.Idle;
            }
        }
